  ...
  if (surface->wl_subsurface == NULL)
  ...
  else
    {
      if (surface->sub.synchronous)
        ...
        return is_surface_effctively_synchronized (surface->sub.parent);
    }
}
...
parent_surface_state_applied (gpointer data, gpointer user_data)
{
  ...
  if (is_surface_effctively_synchronized (surface))
                          ...
                          struct wl_resource *resource)
{
  ...
  was_effectively_synchronized = is_surface_effctively_synchronized (surface);
  surface->sub.synchronous = FALSE;
  if (was_effectively_synchronized &&
      !is_surface_effctively_synchronized (surface))
    apply_pending_state (surface, &surface->sub.pending);
}
