...
typedef enum _MetaCursorGbmBoState
{
  ...
  META_CURSOR_GBM_BO_STATE_FAILED,
...
has_valid_cursor_sprite_gbm_bo (MetaCursorSprite *cursor_sprite)
{
  ...
  switch (cursor_priv->pending_bo_state)
    ...
    case META_CURSOR_GBM_BO_STATE_SET:
      return TRUE;
    case META_CURSOR_GBM_BO_STATE_FAILED:
      return FALSE;
    }
...
static void
set_pending_cursor_sprite_gbm_bo_failed (MetaCursorSprite *cursor_sprite)
{
  MetaCursorNativePrivate *cursor_priv;

  cursor_priv = ensure_cursor_priv (cursor_sprite);
  cursor_priv->pending_bo_state = META_CURSOR_GBM_BO_STATE_FAILED;
}
  ...
  if (width > cursor_width || height > cursor_height)
    {
      ...
      set_pending_cursor_sprite_gbm_bo_failed (cursor_sprite);
      return;
    }
...
static void
destroy_pending_cursor_sprite_gbm_bo (MetaCursorSprite *cursor_sprite)
{
  MetaCursorNativePrivate *cursor_priv =
    g_object_get_qdata (G_OBJECT (cursor_sprite), quark_cursor_sprite);
  guint pending_bo;
