...
{
  return meta_crtc_kms_is_transform_handled (crtc, crtc->transform);
}
               ...
               GpuType                gpu_type)
{
  ...
  const char *primary_gpu_path = NULL;
  ...
  if (gpu_type == GPU_TYPE_SECONDARY)
    primary_gpu_path = meta_gpu_kms_get_file_path (manager_kms->primary_gpu);
  ...
  for (l = devices; l; l = l->next)
    {
      ...
      if (g_strcmp0 (device_path, primary_gpu_path) == 0)
        continue;
                                        ...
                                        GError      **error)
{
  ...
  GList *gpu_paths;
  char *primary_gpu_path;
  GList *l;

  manager_kms->udev = g_udev_client_new (subsystems);

  gpu_paths = get_gpu_paths (manager_kms, GPU_TYPE_PRIMARY);
  ...
  primary_gpu_path = gpu_paths->data;
  ...
  gpu_paths = get_gpu_paths (manager_kms, GPU_TYPE_SECONDARY);
