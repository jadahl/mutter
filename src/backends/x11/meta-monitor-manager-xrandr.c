...
static gboolean
is_output_assignment_unchanged (MetaOutput      *output,
                                MetaCrtcInfo   **crtc_infos,
                                unsigned int     n_crtc_infos,
...
static gboolean
is_assignments_unchanged (MetaMonitorManager *manager,
                          MetaCrtcInfo      **crtc_infos,
                          unsigned int        n_crtc_infos,
                          MetaOutputInfo    **output_infos,
                          unsigned int        n_output_infos)
  ...
  for (i = 0; i < manager->n_crtcs; i++)
    {
      ...
      if (!is_crtc_assignment_unchanged (crtc, crtc_infos, n_crtc_infos))
        return FALSE;
    }
  ...
  for (i = 0; i < manager->n_outputs; i++)
    {
      ...
      if (!is_output_assignment_unchanged (output,
                                           crtc_infos,
                                           n_crtc_infos,
                                           output_infos,
                                           n_output_infos))
  ...
  if (method != META_MONITORS_CONFIG_METHOD_VERIFY)
    {
       ...
       */
      if (!is_assignments_unchanged (manager,
